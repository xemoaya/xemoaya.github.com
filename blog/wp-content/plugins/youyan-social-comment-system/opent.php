...
class MBOpenTOAuth {
  ...
  function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) { 
    $this->sha1_method = new TencentOAuthSignatureMethod_HMAC_SHA1(); 
    $this->consumer = new TencentOAuthConsumer($consumer_key, $consumer_secret); 
    if (!empty($oauth_token) && !empty($oauth_token_secret)) { 
      $this->token = new TencentOAuthConsumer($oauth_token, $oauth_token_secret); 
    } else { 
      $this->token = NULL; 
    } 
  }
  ...
  function getRequestToken($oauth_callback = NULL) { 
    ...
    $token = TencentOAuthUtil::parse_parameters($request); 
    $this->token = new TencentOAuthConsumer($token['oauth_token'], $token['oauth_token_secret']); 
    return $token; 
  } 
  ...
  function oAuthRequest($url, $method, $parameters , $multi = false) { 
    ...
    #echo 66;
    $request = TencentOAuthRequest::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters); 
    #echo 77;
  ...
  function http($url, $method, $postfields = NULL , $multi = false){
    ...
    if($multi){
      #echo 'here3';
      $header .= "Content-Type: multipart/form-data; boundary=" . TencentOAuthUtil::$boundary . "\r\n";	
    }else{	
